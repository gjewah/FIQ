...
class website(models.Model):
    ...
    documentation_builder_public = fields.Boolean(string="Public Documentation", default=False)
    ...
    def create(self, vals_list):
        ...
        for record in result:
            record._generate_docu_menu(record.documentation_builder_public, False)
        return result
    ...
    def _generate_docu_menu(self, shouldbemenu, previouslyexist):
         ...
         * Expected singleton
        """
        exist_ids = self.env["website.menu"].search([("url", "=", "/docs"),  ("website_id", "in", [False, self.id])])
        if shouldbemenu:
