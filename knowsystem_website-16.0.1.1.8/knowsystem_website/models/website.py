...
class website(models.Model):
    ...
    knowsystem_website_portal = fields.Boolean(string="Portal KnowSystem", inverse=_inverse_knowsystem_website_portal)
    ...
    knowsystem_default_sort_id = fields.Many2one("knowsystem.custom.sort", string="Custom Default Sorting")
    ...
    _sql_constraints = [
        ("pager_knowsystem_value_check", "check (pager_knowsystem>0)", _("Articles number must be positive!")),
    ]
    ...
    def create(self, vals_list):
        ...
        Methods:
         * _generate_knowsystem_menu
        """
        result = super(website, self).create(vals_list)
        for record in result:
            record._generate_knowsystem_menu(record.knowsystem_website_public, False)
        return result
