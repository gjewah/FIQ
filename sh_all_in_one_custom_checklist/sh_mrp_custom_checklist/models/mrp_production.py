...
class MRPProduction(models.Model):
    ...
    mrp_complete_state = fields.Selection([("completed", "Completed"),
                                           ("cancelled", "Cancelled")],
    ...
    def _search_state(self, operator, value):
        if operator in ["="]:
            ...
            for rec in self.search([]):
                total_cnt = self.env["mrp.custom.checklist.line"].search_count(
                    [("order_id", "=", rec.id)])
                compl_cnt = self.env["mrp.custom.checklist.line"].search_count(
        ...
        if self:
            for data in self:
                if data.custom_checklist >= 100:
                    data.mrp_complete_state = "completed"
                else:
                    data.mrp_complete_state = "cancelled"

    custom_checklist_ids = fields.One2many("mrp.custom.checklist.line",
                                           "order_id", "Checklist")
    ...
    custom_checklist_template_ids = fields.Many2many(
        comodel_name='mrp.custom.checklist.template',
        relation='mrp_checklist_template_table',
    ...
    def onchange_custom_checklist_template_ids(self):
        ...
        for i in self.custom_checklist_template_ids:
            for j in i._origin.checklist_template_ids:
                new_id = self.env["mrp.custom.checklist.line"].create({
                    'name':
                    j.id,
                    'description':
                    j.description
