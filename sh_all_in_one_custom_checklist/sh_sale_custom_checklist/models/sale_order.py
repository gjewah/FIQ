...
class SaleOrder(models.Model):
    ...
    def _search_state(self, operator, value):
        if operator in ["="]:
            ...
            for rec in self.search([]):
                total_cnt = self.env[
                    "sale.custom.checklist.line"].search_count([
                        ("sale_id", "=", rec.id)
                    ])
                compl_cnt = self.env[
                    "sale.custom.checklist.line"].search_count([
                        ("sale_id", "=", rec.id), ("state", "=", "completed")
                    ])
    ...
    def _compute_custom_checklist(self):
        if self:
            for rec in self:
                total_cnt = self.env[
                    "sale.custom.checklist.line"].search_count([("sale_id",
                                                                 "=", rec.id)])
                compl_cnt = self.env[
                    "sale.custom.checklist.line"].search_count([
                        ("sale_id", "=", rec.id), ("state", "=", "completed")
    ...
    custom_checklist_ids = fields.One2many("sale.custom.checklist.line",
                                           "sale_id", "Checklist",)
    ...
    custom_checklist_template_ids = fields.Many2many(
        'sale.custom.checklist.template', check_company=True)
    ...
    def onchange_custom_checklist_template_ids(self):
        ...
        for i in self.custom_checklist_template_ids:
            for j in i._origin.checklist_template:
                new_id = self.env["sale.custom.checklist.line"].create({
                    'name':
                    j.id,
                    'description':
                    j.description
